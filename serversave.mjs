...
wss.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        ...
        if (msg[0]=='test') {           
            ...
            try {
                socketinvite.send("test,"+msg[1]+","+msg[2])
            }
            catch (e) {
                console.log("error test",e.message,msg[1],msg[2])
                socket.send("PB")
            }
        ...
        if (msg[0]=='OK') {           
            ...
            let socketinvite=global.connectedUsers[msg[2]]
            try {
                socketinvite.send("OK,"+msg[1]+","+msg[2])
            }
            catch (e) {
                console.log("error OK",e.message,msg[1],msg[2])
                socket.send("PB")
            }
